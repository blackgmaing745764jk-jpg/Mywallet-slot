...
// Win Go ဂိမ်းကစားမည့် စာမျက်နှာ
app.get('/play', (req, res) => {
    ...
    <body>
        <div class="card">
            ...
            <div class="colors">
                <button class="green" onclick="selectBet('Green', this)">စိမ်း (Green)</button>
                <button class="violet" onclick="selectBet('Violet', this)">ခရမ်း (Violet)</button>
                <button class="red" onclick="selectBet('Red', this)">နီ (Red)</button>
            </div>
        ...
        <script>
            ...
            // တိုင်မာစနစ် (၃၀ စက္ကန့်တစ်ကြိမ် ရလဒ်ထွက်ရန်)
            let timeLeft = 30;
            setInterval(() => {
                timeLeft--;
...
// လောင်းကြေး လက်ခံခြင်းနှင့် အနိုင်/အရှုံး တွက်ချက်ခြင်း API
app.post('/api/bet', (req, res) => {
    const { gameId, betType, amount } = req.body;
    let currentBal = userBalances[gameId] || 0;

    if (currentBal < amount) {
        return res.json({ success: false, message: 'လက်ကျန်ငွေ မလုံလောက်ပါ။' });
    }

    // ငွေလောင်းလိုက်ပါက လက်ကျန်ငွေထဲမှ ခေတ္တ နှုတ်မည်
    currentBal -= amount;

    // ဥပမာ - စနစ်မှ ကျပန်း (Random) အနိုင်ပေးမည့် နံပါတ် (0 မှ 9 ထိ)
    ...
    userBalances[gameId] = currentBal;
...
const PORT = process.0 || process.env.PORT || 3000;
app.listen(PORT, () => {
