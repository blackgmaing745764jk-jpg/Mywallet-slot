...
// 2. Dashboard / Home Page (ပုံစံတူ Bottom Nav ပါဝင်သော စာမျက်နှာ)
app.get('/home', (req, res) => {
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>CK Lottery Style - ID: ${gameId}</title>
        <style>
            ...
            .banner-sub { background: linear-gradient(135deg, #f59e0b, #fbbf24); border-radius: 8px; padding: 15px; text-align: center; font-weight: bold; color: #1e293b; }
            ...
            .game-card img { width: 100%; height: 60px; object-fit: contain; margin-bottom: 5px; }
            .game-card .g-name { font-size: 13px; font-weight: bold; color: #fbbf24; }
            ...
            .logout-btn { background: #ef4444; color: white; border: none; padding: 8px; width: 100%; border-radius: 5px; margin-top: 20px; font-weight: bold; cursor: pointer; }
        </style>
    </head>
    <body>
        <div class="top-bar">
            <div class="logo">👑 CK LOTTERY</div>
        ...
        <div class="container">
            ...
            <div class="banner-grid-sub">
                <div class="banner-sub">🦈 ငါးဖမ်း</div>
        ...
        <div class="bottom-nav">
            <a href="/home?id=${gameId}" class="nav-item active">
                <div>🏠</div>ပပင်မမျက်နှာပြင်
            </a>
            <a href="/deposit?id=${gameId}" class="nav-item">
                <div>📥</div>ငွေသွင်းရန်
            </a>
            <a href="/withdraw?id=${gameId}" class="nav-item">
                <div>📤</div>ငွေထုတ်ရန်
            </a>
            <a href="/history?id=${gameId}" class="nav-item">
                <div>📋</div>မှတ်တမ်းများ
            </a>
        </div>
...
app.get('/deposit', (req, res) => {
    ...
    <body>
        <div class="container">
            ...
            <a id="tgLink" style="display:none; background:#0ea5e9; color:white; text-align:center; padding:12px; border-radius:5px; margin-top:10px; display:block; text-decoration:none; font-weight:bold;" target="_blank">📲 Telegram သို့ စလစ်ပို့ရန်</a>
        </div>
        <script>
...
// 6. Dedicated History Page (မှတ်တမ်းများကို တစ်သီးတစ်သန့်ခွဲထားခြင်း)
...
// 7. NEW: JILI Mines Game Page (ပုံစံတူ အသစ်ထည့်သွင်းထားသော Mines Game)
app.get('/mines', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .multiplier-bar { display: flex; gap: 5px; overflow-x: auto; padding: 5px 0; margin-bottom: 10px; }
            .m-badge { background: #334155; padding: 5px 10px; border-radius: 4px; font-size: 12px; color: #38bdf8; white-space: nowrap; }
            .m-badge.active { background: #10b981; color: white; font-weight: bold; }
    ...
    <body>
        <div class="card">
            ...
            <div class="top-info">
                ...
                <span>💣 ဗုံး: <b id="mineCount">3</b></span>
            </div>

            <div class="multiplier-bar">
                <div class="m-badge active">1.14x</div>
                <div class="m-badge">1.32x</div>
                <div class="m-badge">1.54x</div>
                <div class="m-badge">1.82x</div>
                <div class="m-badge">2.18x</div>
            </div>

            <div class="grid" id="mineGrid">
                <!-- 25 Tiles Generated via JS -->
            </div>
        ...
        <script>
            ...
            let currentBet = 0;
            let multipliers = [1.14, 1.32, 1.54, 1.82, 2.18, 2.65, 3.28, 4.12];
            let revealedCount = 0;
            ...
            function renderGrid(interactive = false, revealedArr = []) {
                let grid = document.getElementById('mineGrid');
                grid.innerHTML = '';
                for(let i=0; i<25; i++) {
                    let tile = document.div;
            ...
            function startMinesGame() {
                ...
                fetch('/api/mines-start', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ gameId: gameId, amount: amount })
                }).then(res => res.json()).then(data => {
                    if(data.success) {
                        gameActive = true;
                        currentBet = amount;
            ...
            function clickTile(index) {
                ...
                fetch('/api/mines-click', {
                    ...
                    if(data.isBomb) {
                        ...
                        tiles[index].classList.add('revealed-bomb');
                        ...
                        revealedCount++;
                        if(data.winAmount) {
                            document.getElementById('statusText').innerText = 'ရရှိမည့်ငွေ: ' + data.winAmount + ' ကျပ်';
                        }
                    }
                });
            }
...
app.post('/api/mines-start', (req, res) => {
    ...
    db.activeMines[gameId] = { bet: amount, bombs: [3, 12, 19], hits: 0 };
    saveDB(db);
...
app.post('/api/mines-click', (req, res) => {
    ...
    if(session.bombs.includes(tileIndex)) {
        ...
        saveDB(db);
        return res.json({ isBomb: true });
    } else {
        session.hits++;
        let win = Math.floor(session.bet * (1 + (session.hits * 0.2)));
        saveDB(db);
        return res.json({ isBomb: false, winAmount: win });
    }
});
...
app.post('/api/mines-cashout', (req, res) => {
    ...
    let winAmount = Math.floor(session.bet * (1 + (session.hits * 0.2)));
    ...
    delete db.activeMines[gameId];
    saveDB(db);
    res.json({ success: true, message: `ဂုဏ်ယူပါတယ်! ${winAmount} ကျပ် ထုတ်ယူပြီးပါပြီ။` });
});
...
app.get('/admin', (req, res) => {
    ...
    <body style="background:#f1f5f9; font-family:sans-serif; padding:20px;">
        <h2>👑 Admin Panel - ငွေသွင်း/ထုတ်များ</h2>
