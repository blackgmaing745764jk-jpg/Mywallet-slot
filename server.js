...
<body>
    ...
    <script>
        ...
        function openGame(type) {
            ...
            if (type === 'fish') {
                ...
                content.innerHTML = `
                    ...
                    <input type="number" id="crushBetInput" value="10" placeholder="လောင်းမည့် ငွေပမာဏ" style="width: 80%; max-width: 200px; padding: 8px; margin-bottom: 10px; background: #111827; border: 1px solid #4b5563; color: white; border-radius: 6px; text-align: center;">
                    <button style="background: #10b981; color:white; border:none; padding:10px 20px; border-radius:8px; font-weight:bold; cursor:pointer; font-size:15px;" onclick="startCrush()">စတင်မည်</button>
                    <div style="margin-top: 15px; color: #fbbf24; font-weight: bold;">လက်ကျန်ငွေ: <span id="gameBal">${balance}</span> ကျပ်</div>
                `;
            }
        }
        ...
        function startCrush() {
            let bet = Number(document.getElementById('crushBetInput').value);
            if (!bet || bet < 10) { alert('အနည်းဆုံး ၁၀ ကျပ် လောင်းပါ'); return; }
            ...
            let interval = setInterval(() => {
                mult += 0.05;
                let el = document.getElementById('crushMultiplier');
                if (el) el.innerText = mult.toFixed(2) + 'x';

                if (Math.random() < 0.1) {
                    clearInterval(interval);
                    alert('🚀 ပျက်စီးသွားပါပြီ (Crash)! ရှုံးသွားပါပြီ။');
                } else if (mult >= 2.00) {
                    clearInterval(interval);
                    let win = Math.floor(bet * 2);
                    balance += win;
                    updateBalanceUI();
                    alert('🎉 2.00x ဖြင့် အောင်မြင်စွာ ငွေထုတ်ယူနိုင်ing: ' + win + ' ကျပ်');
                }
            }, 200);
        }
    </script>
