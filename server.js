...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .btn-login { background: linear-gradient(135deg, #3b82f6, #1d4ed8); color: #fff; cursor: pointer; font-weight: bold; box-shadow: 0 4px 12px rgba(59,130,246,0.4); }
        </style>
    </head>
    <body>
        <div class="card">
...
app.get('/home', (req, res) => {
    ...
    <head>
        ...
        <style>
            body { background-color: #0f172a; color: #fff; font-family: sans-serif; margin: 0; padding-bottom: 70px; }
            ...
            .game-card { background: linear-gradient(180deg, #1e1b4b, #312e81); border-radius: 12px; padding: 12px 8px; text-align: center; border: 1px solid #4338ca; text-decoration: none; color: white; display: block; box-shadow: 0 4px 8px rgba(0,0,0,0.3); transition: transform 0.1s; }
            ...
            .logout-btn { background: #ef4444; color: white; border: none; padding: 14px; width: 100%; border-radius: 10px; margin-top: 25px; font-weight: bold; cursor: pointer; font-size: 15px; box-shadow: 0 4px 10px rgba(239,68,68,0.3); }
            
            /* Telegram Floating Button CSS */
            .tg-float { position: fixed; bottom: 75px; right: 15px; background: #0ea5e9; color: white; display: flex; align-items: center; padding: 8px 14px; border-radius: 30px; text-decoration: none; box-shadow: 0 4px 15px rgba(14,165,233,0.4); z-index: 1001; font-size: 12px; font-weight: bold; gap: 6px; border: 2px solid #38bdf8; animation: pulse 2s infinite; }
...
app.get('/deposit', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .back-btn { display: inline-block; background: #3b82f6; color: white; padding: 8px 14px; border-radius: 6px; text-decoration: none; margin-bottom: 15px; font-weight: bold; font-size: 13px; }
            input, button { width: 100%; padding: 12px; margin: 8px 0; border-radius: 6px; border: none; font-size: 15px; box-sizing: border-box; }
            ...
            .kpay-box { background: #0f172a; border: 1px dashed #38bdf8; padding: 12px; border-radius: 8px; margin-bottom: 15px; font-size: 13px; text-align: left; }
        </style>
    </head>
    <body>
        <div class="container">
            ...
            <a id="tgLink" style="display:none; background:#0ea5e9; color:white; text-align:center; padding:12px; border-radius:6px; margin-top:10px; text-decoration:none; font-weight:bold; font-size:14px;" target="_blank">📲 Telegram သို့ စလစ်ပို့ရန်</a>
        </div>
        <script>
...
app.get('/history', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .back-btn { display: inline-block; background: #3b82f6; color: white; padding: 8px 14px; border-radius: 6px; text-decoration: none; margin-bottom: 15px; font-weight: bold; font-size: 13px; }
            h3 { color: #f59e0b; border-bottom: 1px solid #334155; padding-bottom: 5px; margin-top: 20px; font-size: 14px; }
        </style>
    </head>
    <body>
        <div class="container">
            <a href="/home?id=${gameId}" class="back-btn">⬅ ပင်မသို့</a>
            <h2 style="font-size: 16px; margin-top: 0;">📋 ကစားသမိုင်းနှင့် ငွေစာရင်းမှတ်တမ်း</h2>
...
// 7. JILI Mines Game Page (Enhanced with Web Audio API sound effects)
app.get('/mines', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .mine-selector { display: flex; justify-content: space-between; align-items: center; background: #334155; padding: 8px 12px; border-radius: 8px; margin-bottom: 10px; font-size: 13px; }
            .mine-selector select { background: #1e293b; color: #fff; border: 1px solid #64748b; padding: 5px 10px; border-radius: 5px; font-size: 13px; font-weight: bold; }

            .grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 6px; margin: 15px 0; }
            .tile { aspect-ratio: 1; background: #415a77; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 22px; cursor: pointer; border: 1px solid #778da9; }
            .tile.revealed-coin { background: #fbbf24; }
            .tile.revealed-bomb { background: #ef4444; }

            input, button { width: 100%; padding: 12px; border-radius: 6px; border: none; font-size: 14px; font-weight: bold; box-sizing: border-box; }
    ...
    <body>
        <div class="card">
            ...
            <p id="statusText" style="color: #38bdf8; font-weight: bold; margin: 5px 0; font-size: 13px;">ငွေလောင်းပြီး စတင်ပါ</p>
...
app.get('/wingo', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .card { background: #1e293b; padding: 15px; border-radius: 12px; max-width: 400px; margin: auto; border: 1px solid #334155; }
            .balance { font-size: 18px; color: #f59e0b; margin: 10px 0; font-weight: bold; }
            .timer { font-size: 26px; color: #ef4444; font-weight: bold; background: #0f172a; padding: 10px; border-radius: 6px; margin: 10px 0; border: 1px solid #334155; }
            input, button { width: 100%; padding: 10px; margin: 8px 0; border-radius: 6px; border: none; font-size: 14px; box-sizing: border-box; }
            input { background: #0f172a; color: #fff; text-align: center; border: 1px solid #475569; }
    ...
    <body>
        <div class="card">
            ...
            <p style="font-size: 13px;">ရွေးချယ်ထားသည်: <b id="chosen" style="color:#f59e0b;">ဘာမှမရွေးရသေးပါ</b></p>
            <input type="number" id="betAmount" placeholder="လောင်းမည့် ငွေပမာဏ">
...
app.get('/fishing', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .top-info { display: flex; justify-content: space-between; align-items: center; background: #334155; padding: 10px; border-radius: 8px; margin-bottom: 10px; font-size: 13px; }
            ...
            .fish-item { position: absolute; cursor: pointer; font-size: 28px; user-select: none; transition: transform 0.2s; }
            ...
            .paytable { background: #0f172a; padding: 8px; border-radius: 6px; font-size: 11px; text-align: left; margin-bottom: 10px; color: #cbd5e1; border: 1px solid #334155; }
    ...
    <body>
        <div class="card">
            ...
            <p id="statusText" style="color: #38bdf8; font-weight: bold; margin: 5px 0; font-size: 12px;">ရေထဲက ငါးများကို နှိပ်၍ ပစ်ပါ</p>
        </div>
...
// Admin Panel (Enhanced with User Balance Direct Adjustment & Status Filtering)
app.get('/admin', (req, res) => {
    let db = loadDB();
    let reqRows = db.requests.map(item => `<tr><td>${item.gameId}</td><td>${item.amount} ကျပ်</td><td><b>${item.status}</b></td><td><button style="background:#10b981; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;" onclick="approveDep('${item.gameId}', ${item.amount}, ${item.reqId})">အတည်ပြု</button></td></tr>`).join('');
    let withRows = db.withdraws.map(item => `<tr><td>${item.gameId}</td><td>${item.amount} ကျပ်</td><td>${item.account}</td><td><b>${item.status}</b></td><td><button style="background:#10b981; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;" onclick="approveWith(${item.reqId})">အတည်ပြု</button></td></tr>`).join('');
    
    let userBalancesHTML = Object.entries(db.balances || {}).map(([id, bal]) => `
        <tr><td>${id}</td><td><b>${bal.toLocaleString()} ကျပ်</b></td><td>
            <input type="number" id="bal_${id}" placeholder="ပမာဏ (+/-)" style="padding:4px; width:100px;">
            <button style="background:#3b82f6; color:white; border:none; padding:5px 8px; border-radius:4px; cursor:pointer;" onclick="adjustBal('${id}')">ပြင်ဆင်မည်</button>
        </td></tr>
    `).join('');
    ...
    <!DOCTYPE html>
    <html lang="my">
    <head><title>Admin Panel - BK77</title></head>
    <body style="background:#0f172a; color:#fff; font-family:sans-serif; padding:20px; max-width:800px; margin:auto;">
