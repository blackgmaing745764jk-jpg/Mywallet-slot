...
bot.start((ctx) => {
    ctx.reply('⭐ BK77 Game Bot သို့ ကြိုဆိုပါတယ်ခင်ဗျာ။ ငွေသွင်း/ငွေထုတ်ရန် ဤဘော့မှတစ်ဆင့် ဆက်သွယ်နိုင်ပါသည်။');
});

bot.command('admin', (ctx) => {
    if (ctx.from.username === MY_TELEGRAM_USER) {
        let db = loadDB();
        ctx.reply(`👑 Admin Panel\n\n- Users Count: ${Object.keys(db.users).length}\n- Pending Deposits: ${db.deposits.length}\n- Pending Withdraws: ${db.withdraws.length}`);
    } else {
        ctx.reply('❌ ဤကွန်းမန်းကို အသုံးပြုခွင့်မရှိပါ။');
    }
});
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                /* Modals / Overlays for Deposit & Withdraw */
                .modal-overlay {
                    display: none;
                    position: fixed;
        ...
        <body>
            ...
            <div class="modal-overlay" id="depositModal">
                <div class="modal-content">
                    <h3>📥 ငွေသွင်းရန်</h3>
                    <p style="font-size: 13px; color: #9ca3af;">KPay / Wave ဖြင့် ငွေလွှဲပြီး ငွေသွင်းရန် တောင်းဆိုပါ။</p>
                    <input type="number" id="depAmount" placeholder="ပမာဏ (ကျပ်) ဥပမာ - 10000">
                    <div class="modal-btns">
                        <button style="background:#374151; color:white;" onclick="closeModal('depositModal')">ပိတ်မည်</button>
                        <button style="background:#10b981; color:white;" onclick="submitDeposit()">တောင်းဆိုမည်</button>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-overlay" id="withdrawModal">
                <div class="modal-content">
                    <h3>📤 ငွေထုတ်ရန်</h3>
                    <p style="font-size: 13px; color: #9ca3af;">ငွေထုတ်မည့် ပမာဏနှင့် ဖုန်းနံပါတ်ကို ထည့်ပါ။</p>
                    <input type="number" id="wdAmount" placeholder="ထုတ်မည့် ပမာဏ (ကျပ်)">
                    <input type="text" id="wdAccount" placeholder="KPay / Wave အကောင့်နံပါတ်">
                    <div class="modal-btns">
                        <button style="background:#374151; color:white;" onclick="closeModal('withdrawModal')">ပိတ်မည်</button>
                        <button style="background:#f59e0b; color:white;" onclick="submitWithdraw()">ထုတ်ယူမည်</button>
                    </div>
                </div>
            </div>
            ...
            <script>
                ...
                function submitDeposit() {
                    ...
                    fetch('/api/deposit', {
                        ...
                        body: JSON.stringify({ uid: '2067', amount: Number(amt) })
                ...
                function submitWithdraw() {
                    ...
                    if(!amt || !acc) { alert('အချက်အလက် အပြည့်အစုံ ဖြည့်ပါ'); return; }
                    fetch('/api/withdraw', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ uid: '2067', amount: Number(amt), account: acc })
                    }).then(res => res.json()).then(data => {
                        alert(data.message);
                ...
                function openGame(type) {
                    ...
                    if (type === 'fish') {
                        ...
                        content.innerHTML = \`
                            <div style="font-size: 50px; margin-bottom: 10px; transition: transform 0.2s;" id="aviatorPlane">✈️</div>
...
// API Routes for Deposit and Withdraw Backend Processing
app.post('/api/deposit', (req, res) => {
    let { uid, amount } = req.body;
    let db = loadDB();
    if (!db.users[uid]) db.users[uid] = { balance: 1000 };
    db.users[uid].balance += amount;
    db.deposits.push({ uid, amount, time: new Date() });
    saveDB(db);
    res.json({ message: 'ငွေသွင်းတောင်းဆိုမှု အောင်မြင်ပါသည်။ လက်ကျန်ငွေ ထည့်သွင်းပြီးပါပြီ။', newBalance: db.users[uid].balance });
});

app.post('/api/withdraw', (req, res) => {
    let { uid, amount, account } = req.body;
    let db = loadDB();
    if (!db.users[uid] || db.users[uid].balance < amount) {
        return res.json({ message: 'ငွေလက်ကျန် မလုံလောက်ပါ။' });
    }
    db.users[uid].balance -= amount;
    db.withdraws.push({ uid, amount, account, time: new Date() });
    saveDB(db);
    res.json({ message: 'ငွေထုတ်ယူမှု အောင်မြင်ပါသည်။', newBalance: db.users[uid].balance });
});
