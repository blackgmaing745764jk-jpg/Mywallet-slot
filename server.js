...
// 2. Dashboard / Home Page (Lobby ထဲမှာတင် ငွေသွင်း/ငွေထုတ် ပါဝင်သည်)
app.get('/home', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .btn-tg { background: #0ea5e9; color: #fff; text-decoration: none; display: block; padding: 10px; margin-top: 5px; font-weight: bold; border-radius: 5px; }
    ...
    <body>
        ...
        <script>
            ...
            // ဘရောက်ဆာ LocalStorage ထဲမှ ငွေလက်ကျန်ကို ထုတ်ပြမည် (ဖျက်မသွားစေရန်)
            let localBal = localStorage.getItem('bal_' + gameId);
            if(localBal !== null) {
                document.getElementById('bal').innerText = localBal;
            } else {
                fetch('/api/balance?id=' + gameId)
                .then(res => res.json())
                .then(data => {
                    document.getElementById('bal').innerText = data.balance;
                    localStorage.setItem('bal_' + gameId, data.balance);
                });
            }
            ...
            function requestWithdraw(id) {
                ...
                .then(data => {
                    ...
                    if(data.success) {
                        ...
                        localStorage.setItem('bal_' + gameId, data.newBalance);
                    }
                });
            }
...
app.get('/play', (req, res) => {
    ...
    <body>
        ...
        <script>
            ...
            let localBal = localStorage.getItem('bal_' + gameId) || 0;
            document.getElementById('bal').innerText = localBal;
            ...
            function placeBet(id) {
                ...
                .then(data => {
                    ...
                    if(data.success) {
                        document.getElementById('bal').innerText = data.newBalance;
                        localStorage.setItem('bal_' + id, data.newBalance);
                    }
                });
            }
        </script>
...
app.post('/api/bet', (req, res) => {
    ...
    if (betType === winningNum.toString() || betType === winningColor || betType === winningSize) {
        isWin = true;
        if (betType === winningNum.toString()) winMultiplier = 9;
