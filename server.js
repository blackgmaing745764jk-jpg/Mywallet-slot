...
// Database Simulation
let users = {
    "BK982104": { name: "Player 1", balance: 1000, phone: "09912345678" }
};
let pendingDeposits = [];
    ...
    <head>
        ...
        <style>
            * { box-sizing: border-box; }
            ...
            .admin-table th, .admin-table td { border: 1px solid #334155; padding: 8px; text-align: left; }
    ...
    <body>
        <div class="app-container">
            <div class="header">
                ...
                <div class="user-info">
                    <div>UID: <span id="userUidDisplay">BK982104</span></div>
                    <div>လက်ကျန်: <span id="userBalance">1000</span> Ks</div>
                </div>
            </div>
            ...
            <div class="main-content">
                <div class="wallet-card">
                    <div>
                        ...
                        <div style="font-size: 18px; font-weight: bold; color: #fff;" id="walletBal">1000 Ks</div>
                    </div>
        ...
        <div class="admin-container" id="adminPanel">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                <h2 style="color:#fbbf24; margin:0;">⚙️ Admin Control Panel</h2>
                <button onclick="closeAdmin()" style="background:#dc2626; color:#fff; border:none; padding:8px 14px; border-radius:8px; font-weight:bold; cursor:pointer;">ပိတ်မည်</button>
            </div>
            
            <div style="background:#1e293b; padding:15px; border-radius:12px; margin-bottom:15px;">
                <h3 style="margin-top:0; font-size:14px; color:#38bdf8;">ကစားသူ ငွေဖြည့်ပေးရန် (Balance Topup)</h3>
                <div style="display:flex; gap:10px; flex-direction:column;">
                    <input type="text" id="admUid" placeholder="UID ထည့်ပါ (ဥပမာ - BK982104)" class="modal-input">
                    <input type="number" id="admAmount" placeholder="ငွေပမာဏ (Ks)" class="modal-input">
                    <button onclick="adminAddBalance()" style="background:#10b981; color:#fff; border:none; padding:10px; border-radius:8px; font-weight:bold; cursor:pointer;">ငွေဖြည့်ပေးမည်</button>
                </div>
            </div>

            <div style="background:#1e293b; padding:15px; border-radius:12px;">
                <h3 style="margin-top:0; font-size:14px; color:#fbbf24;">Pending ငွေသွင်း/ငွေထုတ် တောင်းဆိုချက်များ</h3>
                <div id="adminRequestsList">ဒေတာ မရှိသေးပါ</div>
            </div>
        </div>
        ...
        <div class="modal-overlay" id="walletModal">
            <div class="modal-content">
                ...
                <div class="input-group">
                    <label>UID</label>
                    <input type="text" id="modalUid" class="modal-input" value="BK982104" readonly>
                </div>
                <div class="input-group">
        ...
        <script>
            let currentUid = "BK982104";
            let balance = 1000;
            let currentModalType = '';

            function updateBalanceUI() {
                document.getElementById('userBalance').innerText = balance;
                document.getElementById('walletBal').innerText = balance + ' Ks';
            ...
            function adminAddBalance() {
                let uid = document.getElementById('admUid').value;
                let amt = Number(document.getElementById('admAmount').value);
                if(uid === currentUid) {
                    balance += amt;
                    updateBalanceUI();
                    alert('UID: ' + uid + ' သို့ ' + amt + ' ကျပ် ဖြည့်သွင်းပေးပြီးပါပြီ။');
                } else {
                    alert('UID မတွေ့ရှိပါ။');
                }
            }
            function loadAdminData() {
                let html = '<table class="admin-table"><tr><th>အမျိုးအစား</th><th>UID</th><th>ပမာဏ</th><th>ဖုန်း</th></tr>';
                if(pendingDeposits.length === 0) {
                    html += '<tr><td colspan="4" style="text-align:center;">တောင်းဆိုချက် မရှိသေးပါ။</td></tr>';
                }
                html += '</table>';
            ...
            function submitWallet() {
                ...
                if(currentModalType === 'deposit') {
                    ...
                    pendingDeposits.push({ uid: currentUid, amt: amt, phone: phone });
                    ...
                    if(balance < amt) { alert('ငွေလက်ကျန် မလုံလောက်ပါ။'); return; }
                    balance -= amt;
                    alert('ငွေထုတ်ယူမှု တောင်းဆိုပြီးပါပြီ။');
                }
                ...
                if (type === 'wingo') {
                    ...
                    content.innerHTML = `
                        <div style="text-align:center; width:100%; display:flex; flex-direction:column; align-items:center;">
                            ...
                            <!-- Win Go Betting Options: Size, Colors, Numbers -->
            ...
            function playWingo(choice) {
                ...
                // Dim buttons immediately after betting
                let allBtns = document.querySelectorAll('#activeGameContent button');
                allBtns.forEach(b => { if(b.id && b.id.startsWith('wgBtn')) b.style.opacity = '0.4'; });
                
                document.getElementById('wgStatus').innerText = `ရွေးချယ်ထားသည်: [${choice}] | အချိန်ကုန်သည်အထိ စောင့်ဆိုင်းနေပါ...`;

                // Simulate result evaluation when timer hits next round or after brief delay
                setTimeout(() => {
                    ...
                    if(isWin) {
                        ...
                        balance += winAmt;
                        document.getElementById('wgStatus').innerText = `ထွက်လာသည်: ဂဏန်း(${randNum}) - အရောင်(${randColor}) - (${randSize}) | နိုင်ပါသည်! +${winAmt} ကျပ်`;
                    } else {
